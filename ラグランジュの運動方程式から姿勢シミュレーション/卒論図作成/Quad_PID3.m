...
figure(12); clf; % figure(1)を作成またはクリア
...
function dCurrent = system_dynamics(t, Current, Simu_Val,offset_value, const2, fdX, fR_BtoI, fP_BtoI,time_constant)
    ...
    % 100Hzで制御する。
    persistent cot_time u_log Eta_error_log dEta_error_log cot_index; % persistent 変数の宣言
    N = 100;
    if isempty(cot_time)
        cot_time = 0;
        cot_index = 1;
        Eta_error_log = zeros(N, 3);
        dEta_error_log = zeros(N, 3);
        u_log = kp .* (etad - Eta) - kd .* dEta;
    end
      
tf = floor(t * 100);
if (tf - cot_time) > 0
    % 遅延ステップを指定（例えば0.05秒前のデータを取得するなら5）
    delay_step = 12;
    idx = mod(cot_index - delay_step, N) + 1; % 過去データのインデックス計算
    
    % 過去データを使用
    Eta_error_delayed = Eta_error_log(idx,:)';
    dEta_error_delayed = dEta_error_log(idx,:)';
    
    % 制御入力の更新
    u0 = kp .* (Eta_error_delayed) - kd .* dEta_error_delayed;
    u = u_log + 0.01 * (u0 - u_log) / time_constant;
    
    % バッファに新しいデータを保存
    cot_index = mod(cot_index, N) + 1;

    Eta_error_log(cot_index,:) = (etad - Eta)';
    dEta_error_log(cot_index,:) = dEta';
