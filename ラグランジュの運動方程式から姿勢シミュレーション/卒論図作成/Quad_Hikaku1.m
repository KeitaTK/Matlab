...
plot(real_time, real_att(:,1)*180/pi(), 'b-', 'LineWidth', 1.5); % 実験値 (青実線)
hold on;
plot(ta, Current(:,4)*180/pi(), 'r--', 'LineWidth', 1.5); % シミュレーション値 (赤破線)
hold off;
...
plot(real_time, real_att(:,2)*180/pi(), 'b-', 'LineWidth', 1.5); % 実験値 (青実線)
hold on;
plot(ta, Current(:,5)*180/pi(), 'r--', 'LineWidth', 1.5); % シミュレーション値 (赤破線)
hold off;
% title('ピッチ角 (θ)');
xlabel('Time [s]','FontSize',13);
ylabel('Pitch Angle [degree]','FontSize',12);
legend('Experimental Data', 'Simulation Data','FontSize',11, 'Location', 'NorthWest');

grid on;

% ヨー角 (ψ)
subplot(3, 1, 3); % 3行1列の3番目のsubplot
plot(real_time, real_att(:,3)*180/pi(), 'b-', 'LineWidth', 1.5); % 実験値 (青実線)
hold on;
plot(ta, Current(:,6)*180/pi(), 'r--', 'LineWidth', 1.5); % シミュレーション値 (赤破線)
hold off;
% title('ヨー角 (ψ)');
xlabel('Time [s]','FontSize',13);
