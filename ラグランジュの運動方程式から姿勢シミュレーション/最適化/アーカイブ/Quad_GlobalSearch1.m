...
optionsGS = optimoptions('fmincon', ... % fmincon用のオプション
    ...
    'MaxFunctionEvaluations', 5000, ... % 最大関数評価回数
    'OptimalityTolerance', 1); % 最適性許容誤差
...
gs = GlobalSearch( ...
    ...
    'MaxTime', 3600, ... % 最大実行時間（秒）
    'Display', 'final'); % 最終結果のみ表示
...
Simu_Val_optimized = [params_estimated(1:3); const1; params_estimated(4:5)]; % const1の正しい更新方法
offset_value_optimized = params_estimated(6:7);
...
function error = objective_function(params, objective_args)
    ...
    [ta, Current] = ode45(@(t, Current) system_dynamics(t, Current, Simu_Val,offset_value, const2, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv), tspan, Current0, options);
