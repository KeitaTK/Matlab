...
OF_x = 0.0001; OF_y = 0.0001;
pre_palam = [J_xx; J_yy; J_zz; mu_1; mu_2; OF_x; OF_y];
...
params_estimated = lsqnonlin(@(params) objective_function(params, tspan, Current0, const1, const2, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv, real_time, real_att), params0,[],[],options);
disp('最適化前のパラメータ');
disp(params0);
disp('最適化した後のパラメータ ');
...
const_optimized = [params_estimated(1:3);const1;params_estimated(4:7)];
options = odeset('RelTol',1e-6,'AbsTol',1e-6);
[ta, Current] = ode45(@(t, Current) system_dynamics(t, Current, const_optimized, const2, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv), tspan, Current0,options);
...
% system_dynamics
function dCurrent = system_dynamics(t, Current, const1, const2, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv)
    ...
    vars = {[X_I; Q_I; const1(1:7)]};
    fdx_val = fdX(vars{:});
    dCurrent = fdx_val;
end

function error = objective_function(params, tspan, Current0, const1, const2, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv, real_time, real_att)

    const1 = [params(1:3); const1; params(4:7)];  %paramsとを変数constに代入
    options = odeset('RelTol',1e-6,'AbsTol',1e-8);
    [ta, Current] = ode45(@(t, Current) system_dynamics(t, Current, const1, const2, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv), tspan, Current0,options);
