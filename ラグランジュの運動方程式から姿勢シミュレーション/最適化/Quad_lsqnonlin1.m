...
params_estimated = lsqnonlin(@(params) objective_function(params, tspan, Current0, const, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv, real_time, real_att), params0,[],[],options);
disp('最適化前のパラメータ');
disp(params0);
disp('最適化した後のパラメータ ');
...
[ta, Current] = ode45(@(t, Current) system_dynamics(t, Current, const_optimized, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv), tspan, Current0,options);
...
% system_dynamics
function dCurrent = system_dynamics(t, Current, const, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv)
...
function error = objective_function(params, tspan, Current0, const, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv, real_time, real_att)

    const = [params(1:3);const;params(4:5)];  %paramsとを変数constに代入
    options = odeset('RelTol',1e-6,'AbsTol',1e-8);
    [ta, Current] = ode45(@(t, Current) system_dynamics(t, Current, const, fdX, fR_BtoI, fP_BtoI, time_csv, motor_val_csv), tspan, Current0,options);
