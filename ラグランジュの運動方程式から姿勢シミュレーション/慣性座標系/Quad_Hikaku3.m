...
[ta, Current] = ode45(@(t, Current) system_dynamics(t, Current, const, setting_val, fdX, fR_BtoI, fOmega, fP_BtoI,...
                                                             fP_ItoB, time_csv, motor_val_csv, motor_val_prev, tau), tspan, Current0,options);
%% 
% シミュレーション結果の取り出し
...
function dCurrent = system_dynamics(t, Current, const, setting_val, fdX, fR_BtoI, fOmega, fP_BtoI,...
                                                             fP_ItoB, time_csv, motor_val_csv, motor_val_prev, tau)
    ...
    % 一次遅れフィルタの適用
    % 微分方程式：tau * dmotor_val_filtered/dt + motor_val_filtered = motor_val
    dt = 0.01; % 時間ステップ (必要に応じて変更)
    motor_val_prev;
    motor_val_filtered = motor_val_prev + (motor_val - motor_val_prev) * dt / tau;
    ...
    dCurrent = fdx_val;
       ...
       % フィルタ済みのモーター入力を返すための更新
    assignin('base', 'motor_val_prev', motor_val_filtered);
end